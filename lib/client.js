...
var CSS =
  ...
  '.es-capsule{flex:1;min-width:0;max-width:min(34vw,300px);display:flex;align-items:center;gap:4px;padding:4px 6px 4px 12px;background:var(--es-panel);border:1px solid var(--es-line);border-radius:999px;box-shadow:0 1px 2px rgba(22,40,72,.06),0 6px 20px rgba(22,40,72,.10)}' +
...
// Find the first visible leaf element whose trimmed text matches one of the
// given strings (case-insensitive). Returns null when nothing matches.
function findLabel(matchers) {
  var nodes = document.querySelectorAll('div,span,button,a,p');
  for (var i = 0; i < nodes.length; i++) {
    ...
    for (var j = 0; j < matchers.length; j++) {
      ...
      if (text === m || text.indexOf(m) === 0) { hit = true; break; }
    }
    if (!hit) continue;
...
// Anchor the search pill between the "标准模式" label and the "session log"
// button in the webui header (either order). When the row gap is too small
// (<140px) the pill floats below the header row at full width instead of
// being squeezed. Falls back to the top-right corner when 标准模式 is missing.
function positionBar() {
  var bar = document.querySelector('.es-topbar');
  if (!bar) return;
  var panel = document.querySelector('.es-panel');
  var modeLabel = findLabel(['标准模式']);
  if (modeLabel === null) {
    bar.style.left = '';
    bar.style.right = '16px';
    bar.style.top = '12px';
    bar.style.maxWidth = '';
    if (panel) panel.style.top = '66px';
    return;
  }
  var rA = modeLabel.getBoundingClientRect();
  var sessionLog = findLabel(['session log', '会话日志', '会话记录']);
  var rB = sessionLog === null ? null : sessionLog.getBoundingClientRect();
  var barRect = bar.getBoundingClientRect();
  var left, top, maxW = null;
  var below = false;
  if (rB !== null && rB.left > rA.right) {
    // A …… B
    maxW = rB.left - (rA.right + 24);
    left = rA.right + 12;
    top = rA.top + (rA.height - barRect.height) / 2;
  } else if (rB !== null && rA.left > rB.right) {
    // B …… A
    maxW = rA.left - (rB.right + 24);
    left = rB.right + 12;
    top = rB.top + (rB.height - barRect.height) / 2;
  } else {
    left = rA.right + 12;
    top = rA.top + (rA.height - barRect.height) / 2;
  }
  if (maxW !== null && maxW < 140) below = true;
  if (below) {
    var rowBottom = rB !== null && rB.bottom > rA.bottom ? rB.bottom : rA.bottom;
    var rowLeft = rB !== null && rB.left < rA.left ? rB.left : rA.left;
    left = rowLeft;
    top = rowBottom + 10;
    maxW = null;
  }
  ...
  bar.style.maxWidth = maxW === null ? '' : maxW + 'px';
  if (panel) {
    var b2 = bar.getBoundingClientRect();
    panel.style.top = Math.round(b2.bottom + 8) + 'px';
  }
  console.log('[dsh-english-search] A=', Math.round(rA.left), Math.round(rA.top), 'B=', rB ? Math.round(rB.left) + ',' + Math.round(rB.top) : 'none', 'pill=', Math.round(left) + ',' + Math.round(top), 'maxW=', maxW, 'below=', below);
}
...
function apply(ctx) {
  ...
  var mo = new MutationObserver(function () {
    ...
    positionTimer = setTimeout(positionBar, 120);
  });
