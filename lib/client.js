...
// conversation.session.header.top 插槽：固定在聊天框最上面、滚动时不动。
...
function apply(ctx) {
  ...
  // 固定在聊天框最上面：ui-conversation 的 conversation.session.header.top
  // 插槽（会话头部标题行之上、随头部固定不随消息滚动）。
  slots.inject('conversation.session.header.top', function () {
