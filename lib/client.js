...
//
// v0.2.0 —— 改为原生插槽方案：注册进 conversation.session.header.actions
// （order -5，紧随"标准模式"之后），由会话头部布局原生排布，彻底移除
// fixed 覆盖层 / DOM 锚点定位 / MutationObserver。
...
var CSS =
  '.es-search{--es-bg:#f7f9fd;--es-panel:#ffffff;--es-panel-2:#f5f7fc;--es-text:#152033;--es-muted:#6b7a90;--es-line:#e2e8f2;--es-accent:#007AFF;--es-accent-2:#0A84FF;--es-unknown:#e84545;--es-shadow:0 1px 2px rgba(22,40,72,.04),0 8px 28px rgba(22,40,72,.07);display:inline-flex;align-items:center;gap:8px}' +
  ...
  '.es-capsule{display:flex;align-items:center;gap:4px;padding:4px 8px 4px 12px;background:var(--es-panel-2);border:1px solid var(--es-line);border-radius:999px;box-shadow:0 1px 2px rgba(22,40,72,.08),0 2px 10px rgba(22,40,72,.10)}' +
  '.es-capsule:focus-within{border-color:var(--es-accent);box-shadow:0 0 0 3px rgba(0,0,0,.08)}' +
  '.es-capsule input{width:clamp(110px,16vw,240px);padding:7px 0;font-size:15px;border:none;background:transparent;box-shadow:none;outline:none;color:var(--es-text)}' +
  '.es-capsule input::placeholder{color:var(--es-muted)}' +
  '.es-clear{display:inline-flex;align-items:center;justify-content:center;flex:none;width:24px;height:24px;padding:0;border-radius:50%;background:var(--es-panel-2);color:var(--es-muted);border:1px solid var(--es-line);font-size:14px;line-height:1;cursor:pointer}' +
  '.es-clear:hover{color:var(--es-text)}' +
  '.es-go{border-radius:999px;padding:8px 18px;font-size:15px;flex:none;border:1px solid var(--es-accent);background:linear-gradient(135deg,var(--es-accent) 0%,var(--es-accent-2) 100%),linear-gradient(115deg,transparent 30%,rgba(255,255,255,.26) 50%,transparent 70%);background-size:100% 100%,220% 100%;background-position:0 0,130% 0;color:#fff;cursor:pointer}' +
...
function apply(ctx) {
  ...
  // Native header slot: renders in the session header action row right after
  // 标准模式 (agent-preset, order -10) and before the utility actions
  // (Session log etc.) — the app's own layout, no overlay positioning.
  slots.inject('conversation.session.header.actions', function () {
    return slots.register(
      { name: 'conversation.session.header.actions', id: 'english-search', order: -5, label: '英语搜索' },
      SearchBar,
    );
  });
}
