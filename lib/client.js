...
// button in the webui header: 12px after 标准模式, capped so it never covers
// session log. Falls back to right-of-标准模式, then the top-right corner.
function positionBar() {
  ...
  var maxW = null;
  if (sessionLog !== null) {
    var rB = sessionLog.getBoundingClientRect();
    if (rB.left > rA.right) {
      maxW = rB.left - (rA.right + 24);
      if (maxW < 120) maxW = 120; // keep a usable pill even if it slightly crowds
    }
  }
  var barRect = bar.getBoundingClientRect();
  bar.style.left = Math.round(rA.right + 12) + 'px';
  bar.style.top = Math.round(rA.top + (rA.height - barRect.height) / 2) + 'px';
  bar.style.right = 'auto';
  bar.style.maxWidth = maxW === null ? '' : maxW + 'px';
  if (panel) panel.style.top = Math.round(barRect.bottom + 8) + 'px';
}
