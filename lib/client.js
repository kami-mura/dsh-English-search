...
// Anchor the search pill just right of the "标准模式" label in the webui
// header (12px gap, vertically centered). Falls back to the top-right corner
// when the label is missing (other locale, layout change).
function findModeLabel() {
  var nodes = document.querySelectorAll('div,span,button,a,p');
  for (var i = 0; i < nodes.length; i++) {
    var el = nodes[i];
    if (el.children.length) continue;
    if ((el.textContent || '').trim() !== '标准模式') continue;
...
function positionBar() {
  ...
  bar.style.left = Math.round(r.right + 12) + 'px';
  bar.style.top = Math.round(r.top + (r.height - b.height) / 2) + 'px';
  bar.style.right = 'auto';
  if (panel) panel.style.top = Math.round(b.bottom + 8) + 'px';
}
